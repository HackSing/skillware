...
// 默认指向 poc/fixtures；可用 SKILLWARE_LIBRARY 环境变量覆盖（用于实验 C/D 换真实技能库）。
export const LIBRARY_ROOT = process.env.SKILLWARE_LIBRARY
  ? path.resolve(process.env.SKILLWARE_LIBRARY)
  : path.resolve(__dirname, "../fixtures");

export const LIBRARY_ID = "poc";

// 实验 C 的 C0/C1 开关：为 1 时在 MCP initialize 结果里注入激活规则（instructions 字段），否则行为同现状。
export const ACTIVATION_ENABLED = process.env.SKILLWARE_ACTIVATION === "1";
...
// 逗号分隔，经 SKILLWARE_EXCLUDE_DIRS 传入。core 不硬编码任何具体库的目录名。
export const EXCLUDE_DIRS = new Set(
  (process.env.SKILLWARE_EXCLUDE_DIRS || "")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean)
);
