...
async function main() {
  ...
  for (const c of cases) {
    ...
    const names: string[] = parsed.results.map((x: any) => x.name);
    let mark = "";
    if (c.expect) {
      total++;
      const rank = names.indexOf(c.expect);
      if (rank === 0) { hit++; mark = "✅ Top-1"; }
      else if (rank > 0) { hit++; mark = `⚠️ Top-${rank + 1}`; }
      else mark = "❌ miss";
    } else {
    ...
    if (parsed.results.length === 0) console.log("    (no results)");
  }
